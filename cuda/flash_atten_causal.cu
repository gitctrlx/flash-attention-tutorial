...
__global__ void flash_attention_causal_forward_kernel(
...
__global__ void flash_attention_causal_backward_kernel(
    ...
    for (int i = 0; i < Tr; i++) {
        ...
        if (tx < eff_br) {
            for (int s_col = 0; s_col < eff_bc; s_col++) {
                ...
                int g_row = i * Br + tx;
                int g_col = j * Bc + s_col;
                P_ij[tx * Bc + s_col] = (g_row >= g_col) ? p : 0.0f;
            }
        }
...
int main() {
    ...
    flash_attention_causal_forward_kernel<<<grid_f, Br, shared_size_f>>>(
    ...
    flash_attention_causal_backward_kernel<<<grid_b, Bc, shared_size_b>>>(
